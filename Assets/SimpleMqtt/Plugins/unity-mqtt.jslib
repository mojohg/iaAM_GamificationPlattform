// Copyright (C) 2023 Andreas Vogler - All Rights Reserved 
...
var LibraryMqtt = {
	...
	MqttSetOnOpen: function(callback) {
		context.onOpen = callback;
	},

	MqttSetOnMessage: function(callback) {
		context.onMessage = callback;
	},

	MqttSetOnClose: function(callback) {
		context.onClose = callback;
	},

	MqttCreate: function(clientId, url, tls, usr, pwd, cleanSession, debug) {
		var instanceId = ++context.lastId;

		if (context.debug) console.log("[MqttJS] Create: "+UTF8ToString(url)+" "+UTF8ToString(clientId)+" Id: "+instanceId+" Tls: "+tls);	
		
		context.instances[instanceId] = {
			debug: debug > 0
		};

		var instance = context.instances[instanceId];

		if (!Paho) { 
			console.log("[MqttJS] Create: Paho is NULL!"); 
			return -1;
		}

		var client = new Paho.MQTT.Client(UTF8ToString(url), UTF8ToString(clientId));

		if (!client) { 
			console.log("[MqttJS] Create client returned NULL!");	
			return -1;
		}	
		...
		client.onConnectionLost = function(response) {
			if (instance.debug) console.log("[MqttJS] onConnectionLost! "+response.errorCode);
			try {
				//Runtime.dynCall('vii', context.onClose, [ instanceId, response.errorCode ]);
				Module['dynCall_vii'](context.onClose, instanceId, response.errorCode);
			} catch (err) {
				console.log("[MqttJS] onConnectionLost", err);
			}
		};

		// On Message
		client.onMessageArrived = function(message) {
			if (instance.debug) console.log("[MqttJS] onMessageArrived! Topic: "+message.destinationName);

			var topicBytes = lengthBytesUTF8(message.destinationName);
			var topicBuffer = _malloc(topicBytes + 1);
			stringToUTF8(message.destinationName, topicBuffer, topicBytes + 1);

			// Check if payload is binary and handle appropriately
   			var binaryPayload = message.payloadBytes;
			var payloadBuffer = _malloc(binaryPayload.length);
			var payloadSize = binaryPayload.length;
			HEAPU8.set(binaryPayload, payloadBuffer);

			try {
				Module['dynCall_viiii'](context.onMessage, instanceId, topicBuffer, payloadBuffer, payloadSize);
			} catch (err) {
				console.log("[MqttJS] onMessageArrived", err);
			} finally {
				_free(topicBuffer);
				_free(payloadBuffer);
			}
		...
		// Options
		instance.options = {
			useSSL: tls > 0 ? true : false,
			userName: UTF8ToString(usr),
			password: UTF8ToString(pwd),
			reconnect: false,
			cleanSession: cleanSession!=0,
			onSuccess: function() {
				if (instance.debug) console.log("[MqttJS] onSuccess!");	
				try {
					//Runtime.dynCall('vi', context.onOpen, [ instanceId ]);
					Module['dynCall_vi'](context.onOpen, instanceId);
				} catch(err) {
					console.log("[MqttJS] onSuccess", err);
				}
			}
		}
	...
	MqttSetLastWillMessage: function(instanceId, topic, payload, qos, retained) {
		var instance = context.instances[instanceId];
		if (!instance) return 0;

		if (instance.debug)
			console.log("[MqttJS] MqttSetLastWillMessage "+instanceId+" Topic: "+UTF8ToString(topic)+" Payload: "+UTF8ToString(payload)+" QoS: "+qos+" Retained: "+retained);		
				
		var message = new Paho.MQTT.Message(UTF8ToString(payload));
		message.destinationName = UTF8ToString(topic);
		message.qos = qos;
		message.retained = retained != 0;

		instance.options.willMessage = message;
	},

	MqttSendLastWillMessage: function(instanceId) {
		var instance = context.instances[instanceId];
		if (!instance) return -1;
		if (instance.client === null) return -2;

		if (instance.debug)
			console.log("[MqttJS] MqttSendLastWillMessage "+instanceId);		

		if ("willMessage" in instance.options && instance.options.willMessage) {
			instance.client.send(instance.options.willMessage)
		}		
	},	

	MqttRelease: function(instanceId) {
		var instance = context.instances[instanceId];
		if (!instance) return 0;

		if (instance.debug)
			console.log("[MqttJS] Release "+instanceId);		

		if (instance.client !== null) {
			instance.client.disconnect();
		}
		delete context.instances[instanceId];
		return 0;
	},

	MqttConnect: function(instanceId) {
		var instance = context.instances[instanceId];
		if (!instance) {
			if (context.debug) console.log("[MqttJS] Connect: No instance found!");			
			return -1;
		}
		if (instance.client == null) {
			if (instance.debug) console.log("[MqttJS] Connect: No client found!");			
			return -2;
		}

		if (instance.debug)
			console.log("[MqttJS] Connect "+instanceId);

		try {
			instance.client.connect(instance.options);
			return 0;
		} catch(err) {
			console.log("[MqttJS] MqttConnect", err);
			return -4;
		}
	...
	MqttDisconnect: function(instanceId) {
		var instance = context.instances[instanceId];
		if (!instance) return -1;
		if (instance.client === null) return -2;
		
		if (instance.debug)
			console.log("[MqttJS] Disconnect "+instanceId);

		try {				
			instance.client.disconnect();
			return 0;			
		} catch(err) {
			console.log("[MqttJS] MqttDisconnect", err);
			return -4;
		}
	},

	MqttSubscribe: function(instanceId, topicPtr, qos) {
		var instance = context.instances[instanceId];
		if (!instance) return -1;		
		if (instance.client === null) return -2;

		if (instance.debug)
			console.log("[MqttJS] MqttSubscribe "+instanceId+ " Topic: "+UTF8ToString(topicPtr));		

		try {
			instance.client.subscribe(UTF8ToString(topicPtr), { "qos": qos });
		} catch (err) {
			console.log("[MqttJS] MqttSubscribe", err);
			return -4;
		}		
	},

	MqttUnsubscribe: function(instanceId, topicPtr) {
		var instance = context.instances[instanceId];
		if (!instance) return -1;		
		if (instance.client === null) return -2;

		if (instance.debug)
			console.log("[MqttJS] MqttUnsubscribe "+instanceId+ " Topic: "+UTF8ToString(topicPtr));		

		try {
			instance.client.unsubscribe(UTF8ToString(topicPtr));
		} catch (err) {
			console.log("[MqttJS] MqttUnsubscribe", err);
			return -4;
		}		
	},	

	MqttPublishBuffer: function(instanceId, topicPtr, bufferPtr, length, qos, retain) {
		var instance = context.instances[instanceId];
		if (!instance) return -1;		
		if (instance.client === null) return -2;

		if (instance.debug)
			console.log("[MqttJS] MqttPublishBuffer "+instanceId+" Topic: "+UTF8ToString(topicPtr));		

		try {
			instance.client.send(
				UTF8ToString(topicPtr), 
				HEAPU8.buffer.slice(bufferPtr, bufferPtr + length),
				qos,
				retain > 0);
			return 0;			
		} catch (err) {
			console.log("[MqttJS] MqttPublishBuffer", err);
			return -4;
		}		
	},

	MqttPublishString: function(instanceId, topicPtr, strPtr, qos, retain) {
		var instance = context.instances[instanceId];
		if (!instance) return -1;		
		if (instance.client === null) return -2;

		if (instance.debug)
			console.log("[MqttJS] MqttPublishString "+instanceId+" Topic: "+UTF8ToString(topicPtr));			
		...
		try {
			...
			console.log("[MqttJS] MqttPublishString", err);
			return -4;
		}		
	},

	MqttLoadScript: function() {
/*******************************************************************************
...
autoAddDeps(LibraryMqtt, '$context');
mergeInto(LibraryManager.library, LibraryMqtt);